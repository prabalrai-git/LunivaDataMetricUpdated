...
import { dateWiseTestApi, GetListOfPatientDetailsBydateAndTests } from '../../../services/careLabService';
...
const DyReport = () => {
  ...
  const getApiData = (data) => {
    if (data.SearchType === 1) {
      ...
      dispatch(GetListOfPatientDetailsBydateAndTests(data, (res) => {
        if (res.length > 0) {
          let tableKeys = Object.keys(res[0]);
          let data = []
          tableKeys.forEach(ele => {
